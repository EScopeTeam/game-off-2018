...
public class AuthenticationConfig {
  ...
  public JWTAuthOptions getJWTAuthOptions() {
    final JsonObject jwtConfig = config.getJsonObject(KEY_JWT, new JsonObject());

    return new JWTAuthOptions().setKeyStore(new KeyStoreOptions().setPath(jwtConfig.getString(KEY_KEYSTORE_PATH, KEYSTORE_PATH_DEFAULT_VALUE))
        .setPassword(jwtConfig.getString(KEY_PASSWORD, KEY_PASSWORD_DEFAULT))).setJWTOptions(getJWTOptions());
  }

  public JWTOptions getJWTOptions() {
    final JsonObject jwtConfig = config.getJsonObject(KEY_JWT, new JsonObject());

    return new JWTOptions().setAlgorithm(jwtConfig.getString(KEY_ALGORITHM, ALGORITHM_DEFAULT_VALUE))
